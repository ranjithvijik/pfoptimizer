...
class EnhancedUnifiedAnalyzer:
    ...
    def run_enhanced_backtest(self, window=252, rebalance=21, cost_bps=10):
        ...
        self.backtest_metrics = {
            ...
            'sharpe_ratio': sharpe_ratio(strategy_returns['Value']) if len(strategy_returns) > 0 else 0,
            'max_drawdown': max_drawdown(strategy_returns['Value']) if len(strategy_returns) > 0 else 0,
        ...
        with tab3:
            ...
            if analyzer.backtest_res is not None:
                ...
                with col1:
                    st.metric(
                        "Total Return",
                        f"{analyzer.backtest_metrics['total_return']:.2%}"
                    )
                with col2:
                    st.metric(
                        "Sharpe Ratio",
                        f"{analyzer.backtest_metrics['sharpe_ratio']:.2f}"
                    )
                with col3:
                    st.metric(
                        "Max Drawdown",
                        f"{analyzer.backtest_metrics['max_drawdown']:.2%}"
                    )
                with col4:
                    st.metric(
                        "Avg Turnover",
                        f"{analyzer.backtest_metrics['avg_turnover']:.2%}"
                    )
                ...
                if analyzer.backtest_metrics.get('tracking_error') is not None:
                    ...
                    with col5:
                        st.metric(
                            "Tracking Error",
                            f"{analyzer.backtest_metrics['tracking_error']:.2%}"
                        )
                    with col6:
                        st.metric(
                            "Information Ratio",
                            f"{analyzer.backtest_metrics['information_ratio']:.2f}"
                        )
            else:
...
KEY METRICS
===========
Expected Return: {analyzer.attribution['Return'].dot(analyzer.attribution['Weight']):.2%} if analyzer.attribution is not None else 'N/A'
Portfolio Risk: {np.sqrt(analyzer.weights['Weight'].values @ analyzer.returns[analyzer.weights['Ticker']].cov().values @ analyzer.weights['Weight'].values) * np.sqrt(analyzer.freq_scaler):.2%} if analyzer.weights is not None else 'N/A'

BACKTEST RESULTS
===============
Total Return: {analyzer.backtest_metrics.get('total_return', 'N/A'):.2%} if analyzer.backtest_metrics else 'N/A'
